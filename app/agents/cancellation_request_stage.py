"""Агент для стадии отмены записи."""
...
CANCELLATION_REQUEST_STAGE_INSTRUCTION = """# РОЛЬ
...
Для того чтобы ты мог отменить запись клиента, ты должна понимать какую именно запись он хочет отменить. Если ты не видишь ее из контекста, то используй GetClientRecords (сначала попроси уточнить номер телефона есть ты его не знаешь).
...
cancellation_request_agent = create_react_agent(
    model=llm,
    tools=[
        cancel_booking_tool,
    ...
    prompt=CANCELLATION_REQUEST_STAGE_INSTRUCTION,
)
