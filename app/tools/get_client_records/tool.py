"""
Инструмент GetClientRecords для получения записей клиента по номеру телефона
"""
import asyncio
import logging
...
from app.tools.get_client_records.logic import get_client_records_logic
...
class GetClientRecordsInput(BaseModel):
    """Входные параметры для GetClientRecords"""
    phone: str = Field(
        description="Номер телефона клиента в любом формате (например: +79221192369, 8-922-119-23-69, 89221192369)"
    )


@tool(args_schema=GetClientRecordsInput)
def get_client_records_tool(phone: str) -> str:
    """
    ...
    try:
        ...
        if not future_records:
            ...
            for idx, record in enumerate(future_records, 1):
                ...
                result_text += record_info.rstrip() + "\n"
