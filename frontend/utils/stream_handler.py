...
class EventProcessor:
    ...
    def process_events(self) -> None:
        ...
        # Но при HTTP streaming мы получаем dict, а не кортеж
        for event in stream:
            # Если event - это кортеж (message, metadata), распаковываем
            if isinstance(event, tuple):
                message, _ = event
            else:
                # Если event - это dict (из HTTP), используем его как message
                message = event
